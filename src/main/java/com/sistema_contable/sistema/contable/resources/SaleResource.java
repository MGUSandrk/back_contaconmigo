...
public class SaleResource {
    ...
    public ResponseEntity<?> getSalesByDate(
        ...
        try {
            authService.authorize(token);
            List<SaleResponseDTO> sales = saleService.getSalesByDate(month, year);
            return new ResponseEntity<>(sales, HttpStatus.OK);
        } catch (ModelExceptions modelError) {
            System.out.println(modelError.getMessage());
            return new ResponseEntity<>(null, modelError.getHttpStatus());
    ...
    public ResponseEntity<?> getMonthlySalesReportPdf(
            @RequestHeader("Authorization") String token,
            @RequestParam Integer month,
            @RequestParam Integer year,
        ...
        try {
            authService.authorize(token);
            byte[] pdf = saleService.generateMonthlySalesReportPdf(month, year);

            String disposition = download ? "attachment" : "inline";
            String filename = String.format("sales-report-%04d-%02d.pdf", year, month);
