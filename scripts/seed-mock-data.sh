...
balance_account_id() {
  ...
  id="$(get_json /accounts/balance | jq -r --arg name "$name" '.[] | select(.name == $name) | .id' | head -n 1)"
...
payment_type_id() {
  ...
  id="$(get_json /payment-types | jq -r --arg type "$type" '.[] | select(.type == $type) | .id' | head -n 1)"
  if [ -z "$id" ] || [ "$id" = "null" ]; then
...
product_id() {
  ...
  id="$(get_json /products | jq -r --arg name "$name" '.[] | select(.name == $name) | .id' | head -n 1)"
  if [ -z "$id" ] || [ "$id" = "null" ]; then
...
client_id() {
  ...
  id="$(get_json /clients | jq -r --arg fullName "$full_name" '.[] | select(.fullName == $fullName) | .id' | head -n 1)"
  if [ -z "$id" ] || [ "$id" = "null" ]; then
...
create_initial_funding_entry() {
  ...
  post_json /entry/create "{
    \"description\":\"Aporte inicial de capital para medios de pago mock\",
    \"movements\":[
...
main() {
  ...
  create_payment_account "Banco Mock" 6
  create_payment_account "Tarjeta Mock" 6
  ...
  echo "Creating sales..."
  create_sale "$ana_id" "Efectivo" 1 0 "B" "[{\"productId\":$yerba_id,\"quantity\":3},{\"productId\":$cafe_id,\"quantity\":2}]"
  create_sale "$mercado_id" "Transferencia" 1 5 "A" "[{\"productId\":$azucar_id,\"quantity\":5},{\"productId\":$aceite_id,\"quantity\":2}]"
  create_sale "$carlos_id" "Tarjeta" 3 0 "C" "[{\"productId\":$galletitas_id,\"quantity\":6}]"
  create_sale "$ana_id" "Efectivo" 1 10 "B" "[{\"productId\":$cafe_id,\"quantity\":1},{\"productId\":$yerba_id,\"quantity\":4},{\"productId\":$azucar_id,\"quantity\":2}]"
  create_sale "$mercado_id" "Transferencia" 1 0 "A" "[{\"productId\":$aceite_id,\"quantity\":3}]"
  create_sale "$carlos_id" "Tarjeta" 6 0 "C" "[{\"productId\":$galletitas_id,\"quantity\":10},{\"productId\":$yerba_id,\"quantity\":2}]"
